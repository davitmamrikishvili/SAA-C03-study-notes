...
> [!INFO] Definition
> **Amazon EventBridge** is the evolution of CloudWatch Events. It is a **superset** — everything CloudWatch Events can do, EventBridge can do, plus support for **3rd-party** and **custom application** events. AWS recommends migrating to EventBridge.

### 🧩 How It Works
At a high level, EventBridge implements an **"If X happens (or at time Y) → do Z"** pattern.

* **Event Bus**: A stream of events generated by supported sources within an AWS account.
	* **Default Bus**: Every account has a default event bus (this was the only bus available in CloudWatch Events — implicit and unnamed).
	* **Additional Buses**: EventBridge can create **additional event buses** for 3rd-party SaaS integrations or custom applications — a key differentiator from CloudWatch Events.
* **Rules**: Match incoming events (by pattern) or fire on a **schedule** (cron/rate expressions).
* **Targets**: When a rule matches, the event is routed to **one or more targets** (e.g., Lambda, SNS, SQS, Step Functions).
